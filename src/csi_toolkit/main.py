...
def collect_command(args):
    ...
    if args.live_inference:
        ...
        try:
            ...
            print(
                "Error: ML functionality not installed. Install with: pip install -e '.[ml]'",
                file=sys.stderr,
            )
            return 1
...
def process_command(args):
    ...
    try:
        extractor = FeatureExtractor(
            feature_names, labeled_mode=args.labeled, transition_buffer=args.transition_buffer
        )
...
def train_command(args):
    ...
    # Train model
    try:
        output_dir = trainer.train(input_csv=args.input, output_dir=args.output_dir)
...
def inference_command(args):
    ...
    try:
        output_csv = predictor.run_inference(
            input_csv=args.dataset, output_csv=args.output, include_probabilities=args.probabilities
        )
...
def evaluate_command(args):
    ...
    # Create evaluator
    try:
        evaluator = ModelEvaluator(model_dir=args.model_dir, metric_names=metric_names)
    except Exception as e:
        print(f"Error loading model: {e}", file=sys.stderr)
        return 1
    ...
    try:
        evaluator.run_evaluation(
            input_csv=args.dataset, output_json=args.output_json, output_txt=args.output_txt
        )
